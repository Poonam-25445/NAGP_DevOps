...
public class Config {
	...
	/**
	 * The name of the browser (firefox, chrome, ie, safari) used to open the
	 * application URL.
	 */
	...
	public static void initConstants() {
		...
		try {
			...
			ExtentReportsPath = System.getProperty(USER_DIR) + prop.getProperty("ExtentReportsPath")
					.replace("\\", File.separator).replace("${timestamp}", dateTime)
					.replace("${CurrentTestResultPrefix}", CurrentTestResultPrefix);
			ReportTitle = prop.getProperty("ReportTitle");
			ReportName = prop.getProperty("ReportName");
			TestReportFolder = System.getProperty(USER_DIR)
					+ prop.getProperty("TestReports").replace("\\", File.separator);
			ScreenShotsPath = System.getProperty(USER_DIR) + prop.getProperty("ScreenShotsPath")
					.replace("\\", File.separator).replace("${timestamp}", dateTime)
					.replace("${CurrentTestResultPrefix}", CurrentTestResultPrefix);
